 ...
 *
 * This data represents points of interest around Pittsburgh's
 * David L. Lawrence Convention Center and surrounding areas.
 */

import { POIData, POIType } from '../types/poi';
...
export type PittsburghZone = (typeof PITTSBURGH_ZONES)[keyof typeof PITTSBURGH_ZONES];

/**
 * Conference venues - main conference locations
 */
export const conferenceVenues: POIData[] = [
  {
    id: 'dlcc-main',
 ...
 */
export const restaurants: POIData[] = [
  {
 ...
 */
export const attractions: POIData[] = [
  {
    id: 'point-state-park',
    type: POIType.LANDMARK,
 ...
 */
export const coffeeShops: POIData[] = [
  {
 ...
 */
export const infoPOIs: POIData[] = [
  {
    id: 'registration-desk',
    type: POIType.INFO,
    name: 'CJS2026 Registration',
    description: 'Main registration desk for badge pickup and information. Located in DLCC Hall A lobby.',
    position: {
      x: 500,
      y: 310,
      zone: PITTSBURGH_ZONES.CONVENTION_CENTER,
    ...
    isActive: true,
    metadata: {
      services: ['badge pickup', 'schedule info', 'lost and found', 'accessibility assistance'],
  ...
  },
  {
    ...
    type: POIType.INFO,
    name: 'Media Room',
    description: 'Workspace for journalists covering CJS2026. Power outlets, wifi, and quiet work space.',
    position: {
      x: 510,
      y: 320,
      zone: PITTSBURGH_ZONES.CONVENTION_CENTER,
 ...
 */
export const socialPOIs: POIData[] = [
  {
    id: 'networking-lounge',
    type: POIType.SOCIAL,
    name: 'Networking Lounge',
    description: 'Casual meetup space in DLCC for spontaneous conversations. Coffee and light snacks available.',
    position: {
      x: 490,
      y: 330,
      zone: PITTSBURGH_ZONES.CONVENTION_CENTER,
 ...
 * Get POIs by zone
 */
export function getPOIsByZone(zone: PittsburghZone): POIData[] {
  return pittsburghPOIs.filter((poi) => poi.position.zone === zone);
}

/**
 * Get POIs by type
 */
export function getPOIsByType(type: POIType): POIData[] {
  return pittsburghPOIs.filter((poi) => poi.type === type);
}
...
export function getPOIById(id: string): POIData | undefined {
  return pittsburghPOIs.find((poi) => poi.id === id);
}
